...
for event in longpoll.listen():
    ...
    if text_lower.startswith("/astaff"):
        ...
        chat_users = [m["member_id"] for m in members["items"] if m["member_id"] > 0]

        leaders = [uid for uid in management if uid in chat_users]
        seniors = [uid for uid in senior_admins if uid in chat_users]
        juniors = [uid for uid in chat_users if uid not in management and uid not in senior_admins]

        leader_text = "👑 Руководителей нет"
        if leaders:
            leader_text = "👑 Руководители:\n" + "\n".join(
                f"[id{uid}|{get_user_info(uid)[0]} {get_user_info(uid)[1]}]"
                for uid in leaders
            )
