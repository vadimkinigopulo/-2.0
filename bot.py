...
def list_senior():
    ...
    if not senior_admins:
        return "👤 Ст. Администраторов нет.", 0
...
# ================= Главный цикл =================
logger.info("Бот запущен. Работает только в беседах (игнорирует личные сообщения)")

# Переменная для хранения ID беседы (определится при первом сообщении)
WORK_CHAT_ID = None
        ...
        for event in longpoll.listen():
            try:
                if event.type == VkBotEventType.MESSAGE_NEW:
                    ...
                    # Если это личное сообщение (peer_id > 0) - игнорируем
                    if peer_id > 0:
                        logger.info(f"Игнорируем личное сообщение от пользователя {user_id}")
                        continue
                    
                    # Если это беседа (peer_id < 0)
                    if WORK_CHAT_ID is None:
                        # Запоминаем ID первой беседы, откуда пришло сообщение
                        WORK_CHAT_ID = peer_id
                        logger.info(f"Бот будет работать только в беседе с ID: {WORK_CHAT_ID}")
                    elif peer_id != WORK_CHAT_ID:
                        # Если сообщение из другой беседы - игнорируем
                        logger.info(f"Игнорируем сообщение из другой беседы {peer_id}")
                        continue
